...
from soda_data.sdneo.queries import GET_SODA_PROTEINS_FIGURES
...
class SproutDataGenerator:
    ...
    def __call__(self) -> Any:
        ...
        if self.do_soda:
            ...
            self._get_soda_sprout_triplets(caption_proteins_df, uniprot, sentence_model)
    ...
    def _get_soda_sprout_triplets(self, caption_proteins_df, uniprot, sentence_model):
        ...
        Returns:
            None
        """
        caption_proteins_df["protein_str"] = caption_proteins_df["proteins"].apply(lambda x: " ".join(list(set(x))))
        with open(self.soda_sprout_output_file, "w") as file:
            for i in tqdm(range(len(caption_proteins_df))):
                tmp_df = caption_proteins_df[["panel_caption", "proteins"]].copy()
                caption, proteins = tmp_df.iloc[i]
                proteins = proteins.split(";")
                for protein in proteins:
                    if len(uniprot[uniprot["Entry"] == protein]) > 0:
                        anchors = uniprot[uniprot["Entry"] == protein]["anchor"].values
                        if len(anchors) > 0:
                            anchor = anchors[0]

                            # tmp_df["diff_proteins"] = tmp_df["proteins"].apply(lambda x: (set(proteins.split(";")) - set(x.split(";")) == set(proteins.split(";")))).tolist()
                            negative_example = self._get_negative_caption(caption_proteins_df, protein)
                            file.write(
                                json.dumps(
                                    {
                                        "set": {
    ...
    @staticmethod
    def _get_negative_caption(caption_proteins, anchor_protein):
        filtered_df = caption_proteins[~caption_proteins["protein_str"].str.contains(anchor_protein)]
        output = filtered_df.sample(1)["panel_caption"].values[0]
        return output
    ...
    def _get_caption_proteins(self, split_dict: dict) -> list:
        ...
        """
        collection_query = GET_SODA_PROTEINS_FIGURES()
        results = []
        errors = 0
        for result in DB.query(collection_query):
            if split_dict.get(result.data()["doi"].replace("/", "_").replace(".", "-"), "") in ["train", "validation"]:
                ...
                results.append({"panel_caption": panel_caption, "proteins": list(filter(None, result.data()["proteins"]))})
        return results
